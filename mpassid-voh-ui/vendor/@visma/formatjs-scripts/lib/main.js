...
async function main() {
    ...
    const resultAsString = await extract(await fg([`${appDirectory}/**/*.{j,t}s{,x}`, `!**/*.d.ts`]), {
    ...
    for (const locale of locales) {
        ...
        const files = isPseudoLocale || locale === defaultLocale
            ? defaultLocaleFiles
...
const getFiles = (dependencyPaths, locale) => dependencyPaths
